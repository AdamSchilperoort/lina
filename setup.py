...
# Location of the C++ source tree. The repo root IS the package root, so the
# C++ sources live at ./cpp. Allow override via env var so a user can point
# at a separate cpp checkout.
CPP_SOURCE_DIR = pathlib.Path(
    os.environ.get("LINA_CPP_SOURCE_DIR", HERE / "cpp")
).resolve()
