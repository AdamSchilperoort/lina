...
from lina_cpp._pyref import utils, props, dm
...
def _safe_matmul(a, b):
    try:
        ...
        return xp.asarray(ensure_np_array(a) @ ensure_np_array(b))
...
def _safe_matmul3(a, b, c):
    try:
        ...
        return xp.asarray(
            ensure_np_array(a) @ ensure_np_array(b) @ ensure_np_array(c)
        )
...
class MODEL():
    ...
    def forward(
        ...
        mft_command = _safe_matmul3(self.Mx_dm, dm_command, self.My_dm)
...
def val_and_grad(
    ...
    dJ_dA = _safe_matmul3(M.Mx_dm_back, x1_bar, M.My_dm_back) / ( M.Nsurf * M.Nact * M.Nact ) # why I have to divide by this constant is beyond me
...
def dm_val_and_grad(
    ...
    dJ_dA1 = _safe_matmul3(M.Mx_dm_back, x1_bar, M.My_dm_back) / ( M.Nsurf * M.Nact * M.Nact )
