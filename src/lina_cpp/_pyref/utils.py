...
def create_zernike_modes(pupil_mask, nmodes=15, remove_modes=0, return_np=False):
    ...
    zernikes = xp.asarray(ensure_np_array(zernikes))
    if return_np:
...
def lstsq(modes, data):
    ...
    try:
        ...
        # CuPy/cuSOLVER can intermittently fail on some CUDA stacks.
        # Fall back to a NumPy solve, then lift back into the active backend.
        c_np, *_ = np.linalg.lstsq(
            ensure_np_array(modes),
            ensure_np_array(data),
            rcond=None,
        )
        c = xp.asarray(c_np)
    return c
