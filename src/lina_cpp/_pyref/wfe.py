...
from lina_cpp._pyref import utils
from lina_cpp._pyref.utils import create_zernike_modes
...
def _to_xp_array(arr):
    # Normalize poppy output into numpy first, then lift into active backend.
...
def generate_opd(
    ...
    circ = _to_xp_array(circ)
    bmask = circ>0
    
    wfe_opd = _to_xp_array(wfe_opd)

    if remove_modes>0:
        Zs = _to_xp_array(
            poppy.zernike.arbitrary_basis(
                ensure_np_array(circ),
                nterms=remove_modes,
                outside=0,
            )
        )
...
def generate_wfe(
    ...
    diam = 10*u.mm
    opd_seed = int(opd_seed)
    amp_seed = int(amp_seed)
    wf = poppy.FresnelWavefront(beam_radius=diam/2, npix=npix, oversample=oversample, wavelength=wavelength)
