...
def compute_contrast(ni_im, mask, verbose=True):
    ...
    Returns a scalar contrast, matching ``lina.coro_utils.compute_contrast``.
    """
    contrast, n_mask, n_positive = _core.compute_contrast(
        ensure_np_array(ni_im).astype(np.float64, copy=False),
        ensure_np_array(mask).astype(np.uint8, copy=False),
    )
    if verbose:
        ratio = n_positive / n_mask if n_mask else 0.0
        print(
            f"\tRatio of pixels greater than zero versus total pixels: "
            f"{n_positive} / {n_mask} = {ratio:.2f}"
        )
    return contrast
...
from lina_cpp._pyref.coro_utils import (  # noqa: E402, F401
    add_dm,
    get_cam_exp_time,
    get_cam_gain,
    get_fiber_atten,
